...
graph TD
    ...
    subgraph "1. 프론트엔드 / 데이터 API"
        UI(Streamlit UI)
        API(FastAPI Server)
    end
    
    UI -- "A. 가게 정보 로드" --> API
    UI -- "B. 컨설팅 요청" --> Orchestrator(🧠 Orchestrator)

    subgraph "2. AI 에이전트 시스템"
        Orchestrator
        LLM_Route[LLM (Gemini)<br/>(도구 선택/답변 생성)]

        subgraph "🔧 Tool 목록"
            Tool_Analyze[가맹점/축제 분석<br/>(profile_analyzer.py)]
            Tool_Recommend[지역 축제 추천<br/>(festival_recommender.py)]
            Tool_Strategy[마케팅 전략 생성<br/>(marketing_strategy.py)]
        end

        LLM_Exec[LLM (Gemini)<br/>(도구 실행/분석)]
        
        subgraph "📚 지식 베이스 (RAG)"
            VectorStore[VectorStore (FAISS)<br/>마케팅/축제 정보]
            Embedding[임베딩 모델<br/>(Huggingface)]
        end
    end

    %% 에이전트 흐름
    Orchestrator -- "C. 도구 선택" --> LLM_Route
    Orchestrator -- "D. 도구 호출" --> Tool_Analyze
    Orchestrator -- "D. 도구 호출" --> Tool_Recommend
    Orchestrator -- "D. 도구 호출" --> Tool_Strategy
    
    %% 도구 실행 흐름 (RAG 또는 LLM 단독)
    Tool_Analyze -- "LLM 호출" --> LLM_Exec
    Tool_Recommend -- "LLM 호출" --> LLM_Exec
    Tool_Strategy -- "LLM 호출" --> LLM_Exec
    
    Tool_Recommend -- "DB 검색" --> VectorStore
    Tool_Strategy -- "DB 검색" --> VectorStore
    VectorStore -- "빌드 시 사용" --> Embedding

    %% 최종 답변 흐름
    Tool_Analyze -- "E. 도구 결과" --> Orchestrator
    Tool_Recommend -- "E. 도구 결과" --> Orchestrator
    Tool_Strategy -- "E. 도구 결과" --> Orchestrator
    Orchestrator -- "F. 최종 답변 조합" --> LLM_Route
    LLM_Route -- "G. 최종 답변" --> Orchestrator
    Orchestrator -- "H. 답변 표시" --> UI

    %% 스타일
    style UI fill:#cfc,stroke:#333,stroke-width:2px
    style API fill:#f9f,stroke:#333,stroke-width:2px
    style Orchestrator fill:#ccf,stroke:#333,stroke-width:2px
    style LLM_Route fill:#fec,stroke:#333,stroke-width:2px
    style LLM_Exec fill:#fec,stroke:#333,stroke-width:2px
    style VectorStore fill:#e6f,stroke:#333,stroke-width:2px
```
---
...
요청하신 내용을 반영하여, `전처리정리.pptx` 파일의 상세한 전처리 과정을 `data/final_df.csv` 생성 부분에 추가했습니다.

`festival_df.csv` (2번 항목)의 경우, 이미 `readme.md`에 `festival_df_first.py`, `second.py`, `processing.py`, `add_keywords.py` 스크립트의 역할이 상세히 기술되어 있어, `pptx`의 내용(신한카드 데이터)을 추가할 부분이 없습니다. 따라서 1번 항목을 상세화하는 데 집중했습니다.
...
* **생성:** `create_final_df.py` 스크립트가 `data/` 폴더 내의 3가지 원본 신한카드 CSV 파일을 정제하고 병합하여 생성합니다.
    * **1. 개별 파일 정제:**
        * `big_data_set1_f.csv` (개요정보): 컬럼명 한글화, '상권' 등 결측치 '미확인'으로 처리.
        * `big_data_set2_f.csv` (월별정보): 컬럼명 한글화, '-999999.9' 값 결측치(NaN)로 변경.
        * `big_data_set3_f.csv` (고객정보): 컬럼명 한글화, '-999999.9' 값 결측치(NaN)로 변경.
    * **2. 데이터 병합:**
        * `set2`(월별정보)와 `set3`(고객정보)를 `['가맹점ID', '기준년월']` 기준으로 병합합니다.
        * 이후 `set1`(개요정보)을 `['가맹점ID']` 기준으로 병합하여 마스터 데이터셋을 구성합니다.
* **가공:**
    * **1. 이상치 처리 (성동구 필터링):**
        * '가맹점주소'에 '성동구'가 포함된 데이터만 필터링합니다.
        * 주소는 '성동구'이나 '상권'이 다른 지역인 경우 '미확인(성동구)'로 라벨을 교정합니다.
        * '상권'이 '미확인'이면서 주소가 '성동구'가 아닌 데이터를 제거합니다.
    * **2. 파생 컬럼 생성 (등급화):**
        * `sales_volume_map`, `customer_count_map`, `operation_period_map` 등의 매핑(mapping) 딕셔너리를 적용합니다.
        * '월매출금액_구간' (예: `1_10%이하`) 같은 원본 데이터를 '매출구간_수준' (예: `매출 최상위`)처럼 AI가 쉽게 이해하고 분석할 수 있는 **파생 컬럼**을 생성합니다.
...
* **생성:** 여러 단계의 스크립트를 거쳐 생성됩니다.
    1.  **데이터 통합 (`festival_df_first.py`):** '한국관광데이터랩'에서 다운로드한 연도별/항목별 원본 CSV 파일들(예: `*_문화관광축제 주요 지표.csv`, `*_성_연령별_내국인_방문자.csv`)을 `glob`으로 찾아 `pd.concat`을 사용해 항목별로 하나의 파일로 통합합니다.
    2.  **형태 변환 (`festival_df_second.py`):** 통합된 "Long" 형식의 데이터(예: `통합_주요_지표.csv`)를 `pivot_table`을 사용해 'Wide' 형식으로 변환합니다. 이 과정에서 `(년도)_(컬럼명)` (예: `2019_전체방문자수`) 형태의 컬럼이 생성됩니다.
    3.  **데이터 가공 (`festival_df_processing.py`):** '대한민국 구석구석' 및 개별 축제 홈페이지에서 수집한 텍스트 데이터(소개, 홈페이지 등)를 병합합니다. 또한, Wide 형식의 통계 데이터를 바탕으로 '주요 방문객 성별/연령', '축제 인기도'(상/중/하), '인기도 추세'(상승/하락) 등 분석용 파생 변수를 생성합니다.
    4.  **AI 키워드 생성 (`feastival_df_add_keywords.py`):** 가공된 `festival_df.csv`의 '소개' 컬럼 내용을 Gemini AI에 전달하여, '타겟 고객', '축제 분위기', '주요 콘텐츠' 등의 핵심 키워드를 LLM이 직접 생성하고 `키워드` 컬럼에 추가합니다.
* **용도:**
    1.  '축제 DB'(`vectorstore/faiss_festival`)를 생성하는 원천 자료(Source)입니다.
    2.  `get_festival_profile_by_name` 도구가 축제 이름을 기반으로 상세 정보를 조회할 때 직접 참조하는 파일입니다.
